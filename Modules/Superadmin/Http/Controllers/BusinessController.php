...
//        }
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('is_active', '@if($is_active == 1) <span class="label bg-green">@lang("business.is_active")</span> @else <span class="label bg-danger">@lang("lang_v1.inactive")</span> @endif')
                ->addColumn('action', function ($row) {
                    $html = '<a href="'.
                            action([\Modules\Superadmin\Http\Controllers\BusinessController::class, 'show'], [$row->id]).'"
                                class="btn btn-info btn-sm btn-xs">'.__('superadmin::lang.manage').'</a>
                            <button type="button" class="btn btn-sm btn-primary btn-xs btn-modal" data-href="'.action([\Modules\Superadmin\Http\Controllers\SuperadminSubscriptionsController::class, 'create'], ['business_id' => $row->id]).'" data-container=".view_modal">'
                    ...
                    if ($row->is_active == 1) {
                        $html .= ' <a href="'.action([\Modules\Superadmin\Http\Controllers\BusinessController::class, 'toggleActive'], [$row->id, 0]).'"
                                    class="btn btn-danger btn-sm btn-xs link_confirmation">'.__('lang_v1.deactivate').'
                                </a>';
                    } else {
                        $html .= ' <a href="'.action([\Modules\Superadmin\Http\Controllers\BusinessController::class, 'toggleActive'], [$row->id, 1]).'"
                                    class="btn btn-success  btn-sm btn-xs link_confirmation">'.__('lang_v1.activate').'
                                </a>';
                    }
                    ...
                    if (request()->session()->get('user.business_id') != $row->id) {
                        $html .= ' <a href="'.action([\Modules\Superadmin\Http\Controllers\BusinessController::class, 'destroy'], [$row->id]).'"
                                    class="btn btn-danger btn-sm btn-xs delete_business_confirmation">'.__('messages.delete').'</a>';
                    }
        ...
        return view('superadmin::business.index_1')
