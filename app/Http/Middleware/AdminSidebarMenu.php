...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // Project
            if (auth()->user()->can('project.create_project') || auth()->user()->can('project.edit_project') || auth()->user()->can('project.delete_project')) {
                $menu->dropdown(
                    __('Project'),
                    function ($sub) {
            ...
            // Asset Management
            if (auth()->user()->can('asset.view') || auth()->user()->can('asset.view_all_maintenance') || auth()->user()->can('asset.view_own_maintenance')) {
                $menu->dropdown(
                    __('Asset Management'),
                    function ($sub) {
            ...
            // Woocommerce
            if (auth()->user()->can('woocommerce.sync_orders') || auth()->user()->can('woocommerce.access_woocommerce_api_settings')) {
                $menu->dropdown(
                    __('Woocommerce'),
                    function ($sub) {
                        $sub->url(
            ...
            // Spreadsheet
            if (auth()->user()->can('access.spreadsheet') || auth()->user()->can('create.spreadsheet')) {
                $menu->dropdown(
                    __('Spreadsheet'),
                    function ($sub) {
                        $sub->url(
